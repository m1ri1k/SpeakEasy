...
const cameraPill = document.querySelector("[data-camera-pill]");
...
let selectedCountdown = Number(localStorage.getItem("speakeasy_video_countdown") || "3");
...
function setStageMessage(heading, copy) {
  if (videoHeading) videoHeading.textContent = heading;
  if (videoCopy) videoCopy.textContent = copy;
}
...
function resetRecording() {
  ...
  setStageMessage(
    "Готово к видеопрактике",
    "Выберите тип практики, нажмите «Начать запись» и разрешите доступ к камере и микрофону."
  );
}
...
function getRecordingSupportError(kind) {
  ...
  if (!window.isSecureContext && !isLocalhost) {
    if (isMobile) {
      return `Запись ${kind} на телефоне через локальный IP требует HTTPS. Для проверки на компьютере откройте http://127.0.0.1:5500/frontend/record-video.html.`;
    }

    return `Для записи ${kind} на этом компьютере откройте страницу через localhost: http://127.0.0.1:5500/frontend/record-video.html. Сейчас открыто через ${window.location.host}.`;
  }
...
async function startRecording() {
  ...
  const supportError = getRecordingSupportError("видео");
  if (supportError) {
  ...
  if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) {
    setError("Браузер не поддерживает запись видео. Попробуйте Chrome или Edge.");
    scenarioSelectionLocked = false;
    renderSelectedScenario();
    return;
  }
  ...
  try {
    ...
    setStageMessage(
      "Запись идет",
      "Смотрите в камеру, держите голову ровно и следите за таймером в нижней панели."
    );
    ...
    setStatus("Готово к записи видео");
    setError(error.name === "NotAllowedError"
      ? "Разрешите доступ к камере и микрофону, чтобы записать практику."
...
function stopRecording() {
  ...
  setStageMessage("Готовим запись", "Собираем видеофайл, чтобы его можно было просмотреть или отправить на анализ.");
  pauseButton.disabled = true;
...
function finalizeRecording() {
  ...
  recordedFile = new File([blob], `speakeasy-video-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.${extension}`, {
    type: mimeType,
  });
  ...
  setStageMessage(
    "Запись готова",
    "Просмотрите видео, сохраните черновик или запустите анализ аудио- и видеометрик."
  );
...
async function uploadRecordedVideo(mode) {
  ...
  setStageMessage(
    ...
    mode === "draft" ? "Черновик появится в списке практик." : "Видео отправляется на обработку. Это может занять немного времени."
  );
...
function renderSelectedScenario() {
  ...
  scenarioOptions.forEach((button) => {
    const isUnsupported = button.dataset.scenarioOption === "podcast";
    const isActive = button.dataset.scenarioOption === selectedScenario;
    button.classList.toggle("is-active", isActive);
    button.setAttribute("aria-pressed", String(isActive));
    button.disabled = locked || isUnsupported;
  });
}
...
function setCountdown(seconds) {
  ...
  localStorage.setItem("speakeasy_video_countdown", String(seconds));
...
draftButton.addEventListener("click", () => uploadRecordedVideo("draft"));
processButton.addEventListener("click", () => uploadRecordedVideo("process"));
